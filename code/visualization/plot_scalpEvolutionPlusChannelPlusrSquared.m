function H= plot_scalpEvolutionPlusChannelPlusrSquared(erp, erp_r, mnt, clab, ival, varargin)
%PLOT_SCALPEVOLUTIONPLUSCHANNELPLUSRSQUARED - Display evolution of scalp topographies 
%
%Synposis:
% H= plot_scalpEvolutionPlusChannelPlusrSquared(ERP, ERP_r, MNT, CLAB, IVAL, <OPTS>)
%
%Description:
...
% The last row of the figure is the rSquared value
%
%Input:
% ERP: struct of epoched EEG data.
% ERP_r: struct of epoched r-squared data 
...
%
%See also plot_scalpEvolution, plot_scalpEvolutionPlusChannel, plot_scalpPatternsPlusChannel, plot_scalp.

% Author(s): Benjamin Blankertz, Jan 2005
% Editted by Atieh Bamdadian, April 2015

props= {'ChannelAtBottom', 0,                 'BOOL';
        'FigureColor',     [1 1 1],           'DOUBLE[3]';
        'GlobalCLim',      0,                 'BOOL||DOUBLE[2]';
        'RsquareCLim',     'auto',            'CHAR||DOUBLE[2]'; 
...
opt_rSquared= opt_proplistToStruct(varargin{:});
[opt_rSquared, isdefault]= opt_setDefaults(opt_rSquared, props);
opt_checkProplist(opt_rSquared, props, props_scalpPattern, props_channel);

if isfield(erp_r, 'xUnit'),
  [opt_rSquared,isdefault]= opt_overrideIfDefault(opt_rSquared, isdefault, ...
                                         'XUnit', erp_r.xUnit);
end
if isfield(erp_r, 'yUnit'),
  [opt_rSquared,isdefault]= opt_overrideIfDefault(opt_rSquared, isdefault, ...
                                         'YUnit', erp_r.yUnit);
end

opt_scalpPattern_rSquared= opt_substruct(opt_rSquared, props_scalpPattern(:,1));
opt_channel_rSquared= opt_substruct(opt_rSquared, props_channel(:,1));
...
if opt.PlotChannel && ~isempty(clab),
  if ~isempty(opt.SubplotChannel),
    ...
    if opt.ChannelAtBottom,
      H.ax_erp= subplotxl(1+1+nClasses, 1, 1+nClasses, ...
                          [0.1 0.05 0.1], [0.09 0 0.05]);
    else
      H.ax_erp= subplotxl(1+1+nClasses, 1, 1, 0.05, [0.09 0 0.05]);
      subplot_Offset= 1;
    end
  end
  ...
  H.channel= plot_channel(erp, clab, opt_channel, 'Legend',0,'FontSize',14);
...
for cc= 1:nClasses,
  ...
  for ii= 1:nIvals,
    ...
    if ~isempty(opt.Subplot),
      ...
      subplotxl(1+nClasses+opt.PlotChannel, nIvals, ...
...
for ii= 1:nIvals,
    ...
    if ~isempty(opt.Subplot),
        ...
        subplotxl(1+nClasses+opt.PlotChannel, nIvals, ...
            ii+(cc+subplot_Offset)*nIvals, ...
            [0.01+0.08*cb_per_ival 0.03 0.05], [0.05 0.02 0.1]);
    end
    opt_scalpPattern_rSquared= setfield(opt_scalpPattern_rSquared, 'ScalePos','none');
    opt_scalpPattern_rSquared= setfield(opt_scalpPattern_rSquared, 'Class',1);%size(erp_r.className,2)
    H_rSquared.scalp(1,ii)= plot_scalpPattern(erp_r, mnt, ival(ii,:), opt_scalpPattern_rSquared);
    colormap(H_rSquared.scalp(1,ii).ax,opt.RsquareColormap)
end

if strcmp(opt_rSquared.ScalePos, 'vert'),
    H_rSquared.cb(1)= plotutil_colorbarAside;
    ylabel(H_rSquared.cb(1), opt_rSquared.YUnit);
end
if strcmp(opt.RsquareCLim,'auto')
    clim=[-max(max(abs(erp_r.x))),max(max(abs(erp_r.x)))];% helps to get unified rSquared derived from all channels
else
    clim = opt.RsquareCLim;
end
...
H_rSquared.text= text(0.01, yy, erp_r.className);
set(H_rSquared.text, 'verticalAli','top', ...
    'horizontalAli','center', ...
    'rotation',90, ...
    'Visible','on', ...
    'FontSize',12, ...
    'fontWeight','bold');% you may increase the font size from 10 to 12 to be the same as other captions 
if nargout<1,
  clear H
end