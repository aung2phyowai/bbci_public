function H= custom_grid_plot_tf(epo, mnt, varargin)
...
%  .isDGT       - specify if the time-frequency data is obtained through
%                 the discrete gabor transform. if isDGT is true, we expect
%                 that epo has a field "a" and "M".
%
%Output:
...
if opt.isDGT
    misc_checkType(epo,'!STRUCT(a M)');
    opt = opt_overrideIfDefault(opt,isdefault,'YUnit','Hz');
end
...
if(datadim == 4)
    ...
    %recursively call custom_grid_plot on the dat struct
    %boiled down to one class.
    if nClasses > 1
        H = {};
        for ii = 1:nClassses
            className = epo.className{ii};
            dat_class = dat;
            dat_class.className = [];
            idx_class = dat_class.y(ii,:);
            
            dat_class.y = dat_class.y(ii,:);
            dat_class.x = dat_class.x(:,:,:,idx_class);
            
            figure,
            H_class = custom_grid_plot_tf(epo, mnt, varargin);
            H = {H{:},H_class};
        end
...
if(opt.isDGT)
    ...
    nfs = epo.fs/2;
    freqLim = [nfs/M nfs];
    opt_plot = {opt_plot{:},'FreqLim',freqLim};
