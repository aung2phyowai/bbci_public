...
addpath(genpath('/home/andreas/git/bsdlab_toolbox/code/SPoC_implementation/SPoC_public'));
...
% plot source signals
figure
plot_time_courses(zscore(S), 1,'s');
...
p_est = var(s_est); %estimated source power calculated by the variance
...
r = corrcoef([z_te',P']); %z_te is the "true" source band power, P is the power in the sensor space => why correlating them?
r = r(1,2:end);
...
%% plot lambdas: eigenvalue spectrum
...
s_true = zscore(S(:,1)); %zscore normalizes S to have unit variance
...
hold on
box on
bar(r, 'k') %displays the channelwise correlation as a bar
