...
parser.add_argument('-m', '--method', choices=['sparse','dense'], default='sparse', help='type of flow calculation: sparse (Lucas-Kanade) or dense (Farneback)')

args = parser.parse_args()
#seqFile = '/home/henkolk/local_data/kitti/seqs/seq_s03_1-hardtwaldb.txt'
#seqFile = '/home/henkolk/local_data/kitti/seqs/seq_c07_1-erbprinzenstr.txt'
#seqFile = '/home/henkolk/local_data/kitti/seqs/seq_s09_4-b10nord.txt'
...
for seqFile in args.seqFiles:
    ...
    for i,frameFile in enumerate(files[1:]):
        frame = cv2.imread(frameFile) # cap.read()
        ...
        try:
            if args.method == 'sparse':
                ...
                flowNorm = np.linalg.norm(flow) / st.sum()
                
               
            else:
                flow = cv2.calcOpticalFlowFarneback(old_gray, frame_gray, **fb_params)
                flowNorm = np.linalg.norm(flow) / (flow.shape[0] * flow.shape[1])
        except:
            print("error calculating between frames with shapes %s and %s" % (old_gray.shape, frame_gray.shape))
        else:

         
            flowNorms.append(flowNorm)
            if args.display:
                print("norm of flow %1.2f" % flowNorm)
