...
seqNameFpsTupleList = '[';
for seqIdx = 1:size(block_info, 2)
    seqFileName = block_info{1, seqIdx, 1};
    seqFileFps = block_info{1, seqIdx, 2};
    seqFile = fullfile(PROJECT_SETUP.SEQ_DATA_DIR, seqFileName);
    ...
    nameFpsTuple = sprintf('("%s", %d)', seqFile, seqFileFps);
    if seqIdx ~= 1
