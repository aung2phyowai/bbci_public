/*
  ...
  1. state = bbci_acquire32_bv('init', state); 
  2. state = bbci_acquire32_bv('init', param1, value1, param2, value2, ...); 
  3. [data] = acquire_bv(state);
  4. [data, marker_time] = bbci_acquire32_bv(state);
  5. [data, marker_time, marker_descr] = bbci_acquire32_bv(state);
  6. [data, marker_time, marker_descr, state] = bbci_acquire32_bv(state);
  7. bbci_acquire32_bv('close'); 
  8. bbci_acquire32_bv('close', DUMMY); 
  ...
  NOTE: We observed a data loss when bbci_acquire32_bv is called
...
static int getFieldNumber(mxArray *pStruct, const char *fieldname){
  ...
  if(-1 == fieldNumber) {
    ...
    abv_assert(-1 != fieldNumber, "bbci_acquire32_bv: Could not create field for struct.");
  }
...
static double getScalar(const mxArray *pStruct, const char* fieldname) {
  ...
  abv_assert(NULL != pField, "bbci_acquire32_bv: Field dosen't exist.");
  abv_assert(1 == mxIsDouble(pField), "bbci_acquire32_bv: Field is no scalar"); 
  abv_assert(1 == mxGetM(pField), "bbci_acquire32_bv: Field is no scalar"); 
  abv_assert(1 == mxGetN(pField), "bbci_acquire32_bv: Field is no scalar"); 
...
static double* getArray(const mxArray *pStruct, const char* fieldname) {
  ...
  abv_assert(NULL != pField, "bbci_acquire32_bv: Field dosen't exist.");
  abv_assert(1 == mxIsDouble(pField), "bbci_acquire32_bv: Field is no double type"); 
...
static char* getString(const mxArray *pStruct, const char *fieldname) {
  ...
  pField = mxGetField(pStruct, 0,fieldname);
  abv_assert(NULL != pField, "bbci_acquire32_bv: Field dosen't exist.");
  abv_assert(1 == mxIsChar(pField), "bbci_acquire32_bv: Field is not string.");
...
static int compareString(const mxArray *pStruct, const char* compareString) {
  ...
  abv_assert(mxIsChar(pStruct),"bbci_acquire32_bv: Can't compare a none string to a string");
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  ...
  if(mxIsChar(prhs[0]) && 0 == compareString(prhs[0], "close")) { 
    ...
    if(0 == nlhs) { /* no output arguments */
      ...
      if(0 == connected) {
        mexWarnMsgTxt("bbci_acquire32_bv: no open connection to close!");
        return;
      } else {
        abv_close();
        return;
      ...
      mexErrMsgTxt("bbci_acquire32_bv: no output arguments on close connection.");
    }
  }
  ...
  if(1 <= nrhs && mxIsChar(prhs[0]) && 0 == compareString(prhs[0], "init")) {
    ...
    if(!isStructInit) { // if we have no struct init check if it is a property list
      ...
      if(!isListInit) { // no valid init found
        mexErrMsgTxt("bbci_acquire32_bv: init is only allowed with a struct or a property list as an argument.");
      }
    ...
    if(1 == nlhs) { /* one output argument */
      if(0 != connected) {
        mexErrMsgTxt("bbci_acquire32_bv: close the connection first!");
      } else {
        abv_init(nlhs, plhs, nrhs, prhs, isStructInit);
        return;
      }
    } else {
      mexErrMsgTxt("bbci_acquire32_bv: only one output for a create connection.");
    }
  }
  
  /* check if we are in execution path 3 to 6 */
  abv_assert(4 >= nlhs, "bbci_acquire32_bv: Four ouput arguments are maximum.");
  abv_assert(nrhs == 1, "bbci_acquire32_bv: exactly one input argument required");
  abv_assert(mxIsStruct(prhs[0]), "bbci_acquire32_bv: input argument must be struct");
  ...
  if(0 != connected) {
    ...
    mexErrMsgTxt("bbci_acquire32_bv: open a connection first!");
  }
}
...
abv_init(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[], bool isStructInit)
{
  ...
  if (result == IC_OKAY) {
    ...
    abv_assert(lag * (int)getScalar(OUT_STATE,FIELD_FS) == (int)orig_fs,"bbci_acquire32_bv: The base frequency has to be a multiple of the requested frequency.");
    ...
    abv_assert(1 == checkScalar(OUT_STATE, FIELD_RECONNECT, 1), "bbci_acquire32_bv: Reconnect is no scalar.");
    abv_assert(1 == checkArray(OUT_STATE, FIELD_SCALE, 1, nChans, pMsgStart->dResolutions), "bbci_acquire32_bv: Scale is no array or has wrong size.");
    
    chan_sel = (double *) malloc(nChans*sizeof(double));
    for (n = 0;n<nChans;n++) {
        chan_sel[n] = n+1;
    }
    abv_assert(1 == checkArray(OUT_STATE, FIELD_CHAN_SEL, 1, -nChans, chan_sel), "bbci_acquire32_bv: chan_sel is no array.");  
    free(chan_sel);
    ...
    /* check the filters */
    abv_assert(1 == checkArray(OUT_STATE, FIELD_FILT_SUBSAMPLE, 1, lag, filter_buffer_sub), "bbci_acquire32_bv: Subsample filter is no array or has the wrong size.");
    abv_assert(1 == checkArray(OUT_STATE, FIELD_FILT_A, 1, -1, filter_buffer_a), "bbci_acquire32_bv: IIR filter aSubsample filter has the wrong size.");
    abv_assert(1 == checkArray(OUT_STATE, FIELD_FILT_B, 1, -1, filter_buffer_b), "bbci_acquire32_bv: Subsample filter has the wrong size.");
    ...
    abv_assert(getArrayN(OUT_STATE, FIELD_FILT_B) == iirFilterSize, "bbci_acquire32_bv: bFilter and aFilter must have the same size.");
...
abv_getdata(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  ...
  if (result != -1) {
    ...
    abv_assert(checkArray(IN_STATE, FIELD_FILT_SUBSAMPLE, 1, lag), "bbci_acquire32_bv: Resample filter has to be a vector. Resample filter has to correspondent with the sampling rate.");
    ...
    if (ElementSize==2) {
        // mexWarnMsgTxt("bbci_acquire32_bv: 16 bit");
        int16_t *pSrc = pMsgData->nData;
        for(n = 0; n != pMsgData->nPoints * nChans_orig; ++n) {
          pSrcDouble[n] = (double)pSrc[n];
        }
    } else if (ElementSize==4) {
        // mexWarnMsgTxt("bbci_acquire32_bv: 32 bit");
        // int32_t *pSrc = (int32_t*) pMsgData->nData;
        float *pSrc = (float*) pMsgData->nData;
        for(n = 0; n != pMsgData->nPoints * nChans_orig; ++n) {
          pSrcDouble[n] = (double)pSrc[n];
        }
    } else {
        mexErrMsgTxt("bbci_acquire32_bv: Unknown element size");
    }
    ...
    if (nlhs >= 2) {
      ...
      if (nMarkers > 0) {
        ...
        if(0 == strcmp ("numeric", outputTypeDef)) {
          ...
          mexErrMsgTxt("bbci_acquire32_bv: Unknown ouput type.");
        }
        free(outputTypeDef);
        ...
        if (nlhs >= 3) {
          if(1 == outputType) { /* numeric */
            ...
            mexErrMsgTxt("bbci_acquire32_bv: Unknown ouput type for output.");
          }
        }
    ...
    if(1 == reconnect) {
      printf("bbci_acquire32_bv: getData didn't work, reconnecting ");
      ...
      while(IC_OKAY != (result = initConnection(bv_hostname, &pMsgStart))){
        printf("bbci_acquire32_bv: connecting failed, trying again\n");
        free(pMsgData);
      }
      ...
      printf("bbci_acquire32_bv: getData didn't work, closing connection, returning -2\n ");
